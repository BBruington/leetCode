...
const minimumRounds = function(tasks) {
  ...
  for(i=0; i < sortedTasks.length;) {
    let numTimes = 1;
    while(sortedTasks[i + 1] === sortedTasks[i]) {
      numTimes++;
      i++
    }
    if(numTimes === 1) return -1;
    
    let threePairs = Math.floor(numTimes/3);
    if (numTimes % 3 !== 0) threePairs++;
    solution = solution + threePairs;
    i++
  }
  return solution
};